...
def login_screen():
    ...
    if st.button("התחבר"):
        if pwd in PASSWORDS:
            ...
            st.session_state.teacher_id = pwd  # שמירת הסיסמה כמזהה מורה
            st.rerun()
        else:
...
# ==========================================
# 1. בסיס נתונים (SQLite) - עם הפרדת מורים
# ==========================================
def init_db():
    conn = sqlite3.connect('results.db', check_same_thread=False)
    c = conn.cursor()
    # יצירת טבלה עם עמודת teacher_id
    c.execute('''CREATE TABLE IF NOT EXISTS exams
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  teacher_id TEXT,
                  date TEXT, student_name TEXT, subject TEXT, result TEXT)''')
    
    # בדיקה אם העמודה קיימת (למקרה שהבסיס נתונים ישן)
...
def load_from_db(subject_filter="הכל"):
    ...
    if subject_filter == "הכל":
        df = pd.read_sql_query("SELECT date, student_name, subject, result FROM exams WHERE teacher_id = ?", conn, params=(tid,))
    else:
        df = pd.read_sql_query("SELECT date, student_name, subject, result FROM exams WHERE teacher_id = ? AND subject = ?", conn, params=(tid, subject_filter))
    conn.close()
    return df

# ==========================================
# 2. הגדרות AI (עם Temperature 0 לעקביות בציונים)
# ==========================================
def init_gemini():
    ...
    try:
        ...
        # הגדרות ליציבות ועקביות מקסימלית
        generation_config = {
            "temperature": 0.0,
            "top_p": 1,
            "top_k": 1,
...
tab1, tab2, tab3 = st.tabs(["📄 בדיקת מבחן", "📊 ארכיון אישי", "⚙️הגדרות"])
    ...
    with col2:
        ...
        if st.button("🚀בדוק מבחן"):
            if not file or not student_name:
                ...
                with st.spinner("המבחן בבדיקה מדויקת..."):
                    try:
                        img = Image.open(file)
                        model = init_gemini()
                        if model:
                            prompt = f"""
                            משימה: פענח את כתב היד בתמונה עבור התלמיד {student_name}, השווה למחוון וקבע ציון.
                            נושא: {subject}
                            מחוון: {st.session_state.rubric}
                            
                            הוראות בדיקה קפדניות:
                            1. בצע את פענוח כתב היד בלב. אל תציג את הטקסט הגולמי בתשובה.
                            2. חשב את הציון באופן מתמטי ועקבי. אל תשנה את הניקוד בין הרצות שונות.
                            3. אם תשובה חלקית, קבע ניקוד יחסי קבוע.
...
with tab2:
    st.markdown("<div class='glass-card'>",unsafe_allow_html=True)
    st.subheader(f"הארכיון האישי שלך (מורה {st.session_state.teacher_id})")
